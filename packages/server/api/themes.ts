...
import { UserTheme } from '../models/theme' // Импортируем модель темы пользователя
...
export async function setTheme(req: Request, res: Response): Promise<void> {
  ...
  try {
    let userTheme = await UserTheme.findOne({ where: { ownerId: userId } }) // Находим тему пользователя по идентификатору

    if (!userTheme) {
      // Создаем новую запись темы пользователя, если ее нет
      userTheme = await UserTheme.create({ ownerId: userId, theme: theme } as UserTheme)
    } else {
